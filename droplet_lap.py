...
def Seg(cont,canvas_size):
    for conti in cont:
        ...
        cv2.putText(im_cluster,str(cluster_num),centroid,FONT,1.5,(255,0,0),2,cv2.LINE_AA)
...
def Cal(cont,img):
    ...
    for conti in cont:
        ...
        # im_single[canvas == 255] = (0,255,0)

        white_area = np.sum(cv2.bitwise_and(img,canvas)) / 255
        black_area = np.sum(canvas) / 255 - white_area
        M = cv2.moments(conti)
        centroid = (int(M['m10'] / M['m00']), int(M['m01'] / M['m00']))
...
cv2.drawContours(im_cluster, droplets_cluster, -1, (0,0,255), 2)
cv2.drawContours(im_single, droplets_single, -1, (0,0,255), 2)
